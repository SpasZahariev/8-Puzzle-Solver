...
public class AStar {
    ...
    public void doAStar() {
        root.moveCost = 0;
        root.manhattan = manhattanSum(root.towerBlocks);
        openList.add(root);

        while (!openList.isEmpty() && !isSolution) {
            doAStar(openList.poll());
        }
        //Node goal = openList.peek();
        Node goal = null;
        for (Node i : openList) {
            if (i.manhattan == 0) {
                goal = i;
    ...
    private int manhattanSum(ArrayList<Block> blocks) {
        int sum = 0;
        for (Block i : blocks)
    ...
    private void makeSwitches(Node parent, int futureAgentPos, int currentAgentPos) {
        ...
        if(recalculate) {
            ...
            child.manhattan = manhattanSum(child.towerBlocks);
        } else {
        ...
        if(child.manhattan == 0) {
            ...
            //System.out.println("Manhat dist: " + manhattanSum(child.towerBlocks));
            isSolution = true;
            openList.add(child);
            return;
        }
